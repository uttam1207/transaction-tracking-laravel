...
<div class="page-hero">
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3" style="position:relative;z-index:1;">
        <div>
            <h4>{{ $lead->name }}</h4>
            <p>
                <span class="badge bg-{{ $lead->status_color }} me-1">{{ ucfirst($lead->status) }}</span>
                @if ($lead->source)<span class="text-muted small">{{ $lead->source }}</span>@endif
            </p>
        </div>
        <div class="d-flex gap-2">
            @if (!in_array($lead->status, ['won','lost']))
            <button class="btn btn-primary-grad btn-sm px-4" onclick="convertLead({{ $lead->id }})">
    ...
    <div class="col-md-4">
        <div class="card-glass mb-3">
            <div class="d-flex justify-content-between align-items-center px-4 pt-4 pb-2">
                ...
                <button class="act-btn" onclick="toggleEditMode()"><i class="bi bi-pencil"></i></button>
            </div>
            <div class="px-4 pb-4">
                ...
                <form id="editForm" class="d-none" onsubmit="saveEdit(event)">
                    <div class="mb-2"><label class="form-label small">Name <span class="text-danger">*</span></label><input type="text" name="name" class="form-control form-control-sm" value="{{ $lead->name }}" required></div>
                    ...
                    <div class="mb-2"><label class="form-label small">Email <span class="text-danger">*</span></label><input type="email" name="email" class="form-control form-control-sm" value="{{ $lead->email }}" required></div>
                    <div class="mb-2"><label class="form-label small">Phone <span class="text-danger">*</span></label><input type="text" name="phone" class="form-control form-control-sm" value="{{ $lead->phone }}" required></div>
                    <div class="mb-2"><label class="form-label small">Status <span class="text-danger">*</span></label>
                        <select name="status" class="form-select form-select-sm">
                            @foreach ($statuses as $s)<option value="{{ $s }}" @selected($lead->status === $s)>{{ ucfirst($s) }}</option>@endforeach
                        </select>
                    </div>
                    <div class="mb-2"><label class="form-label small">Priority <span class="text-danger">*</span></label>
                        <select name="priority" class="form-select form-select-sm">
                            @foreach ($priorities as $p)<option value="{{ $p }}" @selected($lead->priority === $p)>{{ ucfirst($p) }}</option>@endforeach
                        </select>
                    </div>
    ...
    <div class="col-md-8">
        <div class="card-glass">
            <div class="d-flex justify-content-between align-items-center px-4 pt-4 pb-2">
                ...
                <button class="btn btn-primary-grad btn-sm px-4" data-bs-toggle="modal" data-bs-target="#addActivityModal">
...
function convertLead(id) {
    APP.confirm('Convert Lead', 'Convert this lead to a customer? This will mark it as Won.', () => {
