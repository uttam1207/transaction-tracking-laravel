                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-start mb-2">
                        <div>
                            ...
                            @if (!$wh->is_active)<span class="badge bg-secondary">Inactive</span>@endif
                        </div>
                        <button class="btn btn-sm btn-outline-secondary" onclick="editWarehouse({{ $wh->id }}, @json($wh))"><i class="bi bi-pencil"></i></button>
                    </div>
                    <h6 class="fw-bold mb-1">{{ $wh->name }}</h6>
                    <div class="text-muted small mb-2">
                        @if ($wh->city){{ $wh->city }}@if ($wh->country), {{ $wh->country }}@endif@endif
                    </div>
                    <div class="d-flex gap-3 small">
                        <span><i class="bi bi-geo-alt me-1"></i>{{ $wh->locations_count }} locations</span>
                        @if ($wh->manager)<span><i class="bi bi-person me-1"></i>{{ $wh->manager->name }}</span>@endif
                    </div>
                </div>
                <div class="card-footer bg-transparent border-0 pt-0">
                    <a href="{{ route('admin.warehouses.show', $wh) }}" class="btn btn-sm btn-outline-primary w-100">View Stock</a>
                </div>
            </div>
        </div>
        @empty
...
<div class="modal fade" id="addWarehouseModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title">New Warehouse</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
