...
<div class="page-hero">
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3" style="position:relative;z-index:1;">
        <div>
            <h4>Customer Categories</h4>
            <p>Manage dynamic categories for organising your CRM customers</p>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('admin.crm.index') }}" class="btn btn-sm btn-outline-light px-4">
        ...
        <div class="card-glass h-100" style="border-top:3px solid {{ $cat->color }};">
            <div class="px-4 pt-4 pb-3">
                ...
                <div style="font-size:.78rem;color:#9ca3af;">
                    ...
                    <strong style="color:#374151;">{{ $cat->customers_count }}</strong> customer{{ $cat->customers_count !== 1 ? 's' : '' }}
                </div>
            </div>
            <div class="px-4 py-3 border-top d-flex gap-2">
                ...
                <form method="POST" action="{{ route('admin.crm-categories.destroy', $cat) }}">
                    @csrf @method('DELETE')
                    <button type="button" class="btn btn-sm btn-outline-danger"
                        {{ $cat->customers_count > 0 ? 'disabled title=\'Has customers — cannot delete\'' : '' }}
                        onclick="APP.confirm('Delete', 'Delete category {{ addslashes($cat->name) }}? This cannot be undone.', () => this.closest('form').submit())">
                        <i class="bi bi-trash"></i>
                    </button>
                </form>
                <a href="{{ route('admin.crm.index', ['category' => $cat->name]) }}"
                   class="btn btn-sm btn-outline-primary" title="View customers in this category">
                    <i class="bi bi-eye"></i>
                </a>
            </div>
        </div>
...
<div class="card-glass mt-4 px-4 py-3">
    ...
    <div class="d-flex flex-wrap gap-2">
        @foreach([
            'bi-people','bi-droplet-fill','bi-bag-check','bi-shop','bi-cash-coin','bi-building',
            'bi-heart-pulse','bi-person-fill','bi-patch-check-fill','bi-patch-check','bi-building-x',
            'bi-person-badge','bi-truck','bi-basket','bi-bank','bi-briefcase','bi-star',
            'bi-bookmark','bi-tag','bi-telephone','bi-envelope','bi-gear','bi-clipboard','bi-tools'
        ] as $icon)
    ...
    <div class="mt-2" style="font-size:.72rem;color:#9ca3af;">Click an icon to copy its class name. Use it in the Icon field when adding/editing a category.</div>
</div>
...
<div class="modal fade" id="addCategoryModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius:16px;border:none;">
            <form method="POST" action="{{ route('admin.crm-categories.store') }}">
            @csrf
...
document.querySelectorAll('.edit-cat-btn').forEach(btn => {
    btn.addEventListener('click', function () {
        document.getElementById('editCategoryForm').action = '/admin/crm-categories/' + this.dataset.id;
