...
<div class="page-hero">
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3" style="position:relative;z-index:1;">
        <div>
            ...
            <p><span class="badge bg-{{ $entry->status_color }} me-2">{{ ucfirst($entry->status) }}</span>{{ $entry->entry_date->format('d M Y') }}</p>
        </div>
        <div class="d-flex gap-2">
            @if ($entry->status === 'draft')
            <button class="btn btn-primary-grad btn-sm px-4" onclick="postEntry({{ $entry->id }})">
                <i class="bi bi-check-circle me-1"></i> Post Entry
            </button>
            @endif
            @if ($entry->status === 'posted')
            <button class="btn btn-warning btn-sm px-4" onclick="reverseEntry({{ $entry->id }})">
...
<div class="row g-4">
    ...
    <div class="col-md-4">
        <div class="card-glass p-4">
            <div class="fw-semibold mb-3">Audit</div>
