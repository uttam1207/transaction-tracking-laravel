            ...
            <div class="card-glass p-4">
                ...
                <form action="{{ route('admin.milk.store') }}" method="POST">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            <label class="form-label fw-semibold">Animal <small class="text-muted fw-normal">(leave blank
                                    for batch / full shed)</small></label>
                            <select name="animal_id" class="form-select @error('animal_id') is-invalid @enderror">
                                ...
                                @foreach ($animals as $a)
                                    <option value="{{ $a->id }}" @selected(old('animal_id') == $a->id)>{{ $a->tag_number }} —
                                        {{ $a->name }}</option>
                                @endforeach
                            </select>
                            @error('animal_id')
                        ...
                        <div class="col-12">
                            <label class="form-label fw-semibold">Quantity (Liters) <span
                                    class="text-danger">*</span></label>
                            <input type="number" step="0.1" min="0.1" name="quantity_liters"
                                class="form-control @error('quantity_liters') is-invalid @enderror" placeholder="e.g. 75.5"
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" step="0.01" name="fat_percentage"
                                class="form-control @error('fat_percentage') is-invalid @enderror" placeholder="e.g. 7.8"
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" step="0.01" name="snf_percentage"
                                class="form-control @error('snf_percentage') is-invalid @enderror" placeholder="e.g. 9.0"
                                value="{{ old('snf_percentage', 9.0) }}">
                            @error('snf_percentage')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-4">
                            ...
                            <input type="number" step="0.01" name="clr_value"
                                class="form-control @error('clr_value') is-invalid @enderror" placeholder="e.g. 28.5"
                                value="{{ old('clr_value') }}">
                            @error('clr_value')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
