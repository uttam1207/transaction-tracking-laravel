@extends('layouts.app')
@section('title', 'Deleted Purchase Orders')
...
<div style="background:linear-gradient(135deg,#450a0a,#7f1d1d);border-radius:16px;padding:22px 28px;margin-bottom:24px;color:#fff;">
    <h5 class="mb-1 fw-bold" style="font-weight:800;">Deleted Purchase Orders <span style="background:rgba(255,255,255,.15);border-radius:6px;padding:2px 10px;font-size:.8rem;">{{ $orders->total() }}</span></h5>
    <p class="mb-0" style="opacity:.7;font-size:.82rem;">Soft-deleted POs — ledger entries were reversed on delete. Restore to reverse that effect.</p>
</div>
...
<div style="background:#fff;border:1px solid #e5e7eb;border-radius:14px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.04);">
    <div style="padding:14px 20px;border-bottom:1px solid #f3f4f6;background:#fef2f2;display:flex;align-items:center;gap:8px;justify-content:space-between;">
        ...
        <a href="{{ route('admin.procurement.index') }}" style="font-size:.78rem;color:#4f46e5;text-decoration:none;font-weight:600;">
    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0" style="font-size:.83rem;">
            <thead style="background:#fef2f2;">
                <tr>
                    <th style="padding:11px 16px;font-weight:700;font-size:.72rem;text-transform:uppercase;color:#991b1b;border:none;">PO Number</th>
                    <th style="padding:11px 16px;font-weight:700;font-size:.72rem;text-transform:uppercase;color:#991b1b;border:none;">Vendor</th>
                    ...
                    <th style="padding:11px 16px;font-weight:700;font-size:.72rem;text-transform:uppercase;color:#991b1b;border:none;">Order Date</th>
            ...
            <tr style="border-bottom:1px solid #fef2f2;">
                <td style="padding:11px 16px;font-family:monospace;font-size:.78rem;color:#4f46e5;font-weight:600;">{{ $order->po_number }}</td>
                <td style="padding:11px 16px;">{{ $order->vendor?->name ?? '—' }}</td>
                <td style="padding:11px 16px;font-weight:600;">₹{{ number_format($order->total_amount, 2) }}</td>
                <td style="padding:11px 16px;">
                    <span style="padding:2px 9px;border-radius:5px;font-size:.72rem;font-weight:600;background:#f3f4f6;color:#6b7280;">{{ $order->status }}</span>
                </td>
                <td style="padding:11px 16px;color:#374151;">{{ $order->order_date->format('d M Y') }}</td>
                <td style="padding:11px 16px;color:#9ca3af;font-size:.78rem;">{{ $order->deleted_at->format('d M Y, H:i') }}</td>
                <td style="padding:11px 16px;text-align:right;">
                    <div style="display:flex;gap:6px;justify-content:flex-end;">
                        <form method="POST" action="{{ route('admin.procurement.restore', $order->id) }}"
                              onsubmit="return confirm('Restore this purchase order? Ledger effects will be re-applied.')">
                            @csrf
                        ...
                        <form method="POST" action="{{ route('admin.procurement.force-delete', $order->id) }}"
                              onsubmit="return confirm('Permanently delete {{ $order->po_number }}? Cannot be undone.')">
                            @csrf @method('DELETE')
    ...
    @if($orders->hasPages())
    <div style="padding:14px 20px;border-top:1px solid #f3f4f6;">{{ $orders->links() }}</div>
    @endif
    @endif
</div>
@endsection
