    ...
    <div style="background:linear-gradient(135deg,#059669,#16a34a);padding:20px 28px;position:relative;overflow:hidden;">
        ...
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                ...
                <div style="font-size:1.05rem;font-weight:800;color:#fff;">Sales Invoice Details</div>
            </div>
        </div>
    </div>
    ...
    <div class="p-4">
        ...
        <form action="{{ route('admin.sales.store') }}" method="POST" id="saleForm">
            ...
            <div class="row g-3 mb-3">
                <div class="col-md-4">
                    ...
                    <div class="input-group">
                        ...
                        <button type="button" class="btn btn-outline-secondary" id="regenInvBtn" title="Generate next number">
                            <i class="bi bi-arrow-clockwise" id="regenIcon"></i>
                        </button>
                        @error('invoice_number')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div style="font-size:.68rem;color:#9ca3af;margin-top:3px;">Auto-generated — edit only if needed</div>
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                    ...
                    <select name="payment_terms" id="paymentTerms" class="form-select @error('payment_terms') is-invalid @enderror" onchange="applyPaymentTerms()">
                        ...
                        @foreach(['Due on Receipt','Net 15','Net 30','Net 45','Net 60'] as $pt)
                            <option value="{{ $pt }}" @selected(old('payment_terms')===$pt)>{{ $pt }}</option>
                        @endforeach
                    </select>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-3 d-none" id="amountPaidRow">
...
<script>
(function () {
    /* ── Item types data from server ──────────────────────────────── */
    const TYPES = @json($itemTypes->keyBy('id'));
    /* ── old() values on validation failure ──────────────────────── */
    ...
    document.getElementById('itemsBody').addEventListener('change', function (e) {
        if (e.target.classList.contains('item-type-select')) {
            updateRowType(e.target.closest('tr'));
        }
    });
    ...
    if (OLD_ITEMS && OLD_ITEMS.length > 0) {
        OLD_ITEMS.forEach(item => addRow(item));
    } else {
        addRow();
    }
    ...
    document.getElementById('regenInvBtn').addEventListener('click', function () {
        ...
        fetch('{{ route("admin.sales.next-number") }}')
            ...
            .catch(() => {})
            .finally(() => { btn.disabled = false; icon.style.transform = 'rotate(0deg)'; });
    });
})();
</script>
@endpush
