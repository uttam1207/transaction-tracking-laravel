        ...
        <div class="col-6 col-md-3">
            <div class="card stat-card h-100">
                <div class="card-body">
                    ...
                    <small class="text-success"><i class="bi bi-arrow-up"></i> Today:
                        ${{ number_format($stats['today_revenue'], 0) }}</small>
                </div>
            </div>
        </div>
        <div class="col-6 col-md-3">
        ...
        <div class="col-6 col-md-3">
            <div class="card stat-card h-100">
                <div class="card-body">
                    ...
                    <h4 class="mb-0 fw-bold text-danger">{{ $stats['fraud_alerts_open'] ?? 0 }}</h4>
            ...
            <div class="card stat-card h-100">
                <div class="card-body">
                    <div class="d-flex align-items-start justify-content-between">
                        ...
                        <div class="stat-icon"
                            style="background: rgba(16,185,129,0.1); color: #10b981; width:40px; height:40px; font-size:1.1rem;">
            ...
            <div class="card stat-card h-100">
                <div class="card-body">
                    <div class="d-flex align-items-start justify-content-between">
                        ...
                        <div class="stat-icon"
                            style="background: rgba(245,158,11,0.1); color: #f59e0b; width:40px; height:40px; font-size:1.1rem;">
            ...
            <div class="card stat-card h-100">
                <div class="card-body">
                    <div class="d-flex align-items-start justify-content-between">
                        ...
                        <div class="stat-icon"
                            style="background: rgba(239,68,68,0.1); color: #ef4444; width:40px; height:40px; font-size:1.1rem;">
            ...
            <div class="card stat-card h-100">
                <div class="card-body">
                    <div class="d-flex align-items-start justify-content-between">
                        ...
                        <div class="stat-icon"
                            style="background: rgba(99,102,241,0.1); color: #6366f1; width:40px; height:40px; font-size:1.1rem;">
                ...
                <div class="card-header d-flex justify-content-between align-items-center py-3">
                    ...
                    <div class="btn-group btn-group-sm">
                        ...
                        <button class="btn btn-outline-secondary active"
                            onclick="updateChart('transactions', 30)">30D</button>
        ...
        <div class="col-12" data-widget="quick-stats">
            <div class="card shadow-sm">
                <div class="card-header fw-semibold d-flex justify-content-between">
                    ...
                    <button class="btn btn-xs btn-outline-secondary widget-hide py-0 px-1"><i
                            class="bi bi-dash"></i></button>
                </div>
                <div class="card-body widget-body">
                    <div class="row g-2 text-center">
                        <div class="col-3">
                            <div class="fw-bold text-primary">{{ $stats['total_transactions'] }}</div>
                            <div class="small text-muted">Transactions</div>
                        </div>
                        <div class="col-3">
                            <div class="fw-bold text-success">${{ number_format($stats['total_revenue'], 0) }}</div>
                            <div class="small text-muted">Revenue</div>
                        </div>
                        <div class="col-3">
                            <div class="fw-bold text-danger">{{ $stats['fraud_alerts_open'] }}</div>
                            <div class="small text-muted">Fraud Alerts</div>
                        </div>
                        <div class="col-3">
                            <div class="fw-bold text-info">{{ $stats['total_employees'] }}</div>
                                ...
                                @forelse($recentTransactions as $tx)
                                    <tr>
                                        <td class="px-3">
                                            ...
                                            @if ($tx->is_flagged)
                                                <i class="bi bi-flag-fill text-danger small ms-1"></i>
                                            @endif
                                        </td>
                                        <td><span class="fw-semibold">{{ $tx->currency }}
                                                {{ number_format($tx->amount, 2) }}</span></td>
                                        <td>
                                            <span
                                                class="badge bg-{{ $tx->status_badge }}-subtle text-{{ $tx->status_badge }} border border-{{ $tx->status_badge }}-subtle">
                                                {{ ucfirst($tx->status) }}
                                            </span>
                                        </td>
                                        <td>
                                            <span
                                                class="badge bg-{{ $tx->risk_score >= 70 ? 'danger' : ($tx->risk_score >= 40 ? 'warning' : 'success') }}-subtle
                                                        text-{{ $tx->risk_score >= 70 ? 'danger' : ($tx->risk_score >= 40 ? 'warning' : 'success') }}">
                                                {{ $tx->risk_score }}%
                                            </span>
                                        </td>
                                        <td><span class="small text-muted">{{ $tx->created_at->diffForHumans() }}</span>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                ...
                <div class="card-body">
                    @forelse($topEmployees as $emp)
                        <div class="d-flex align-items-center gap-3 mb-3">
                            <img src="{{ $emp->user->avatar_url }}" class="rounded-circle" width="36"
                                height="36" alt="">
...
@push('styles')
    <style>
        ...
        #widgetGrid [data-widget] {
            cursor: default;
        }

        #widgetGrid.drag-mode [data-widget] {
            cursor: grab;
        }

        #widgetGrid.drag-mode .card {
...
@push('scripts')
    ...
    <script>
        ...
        function getStoredOrder() {
            try {
                return JSON.parse(localStorage.getItem('dashboard_widget_order') || 'null');
            } catch {
                return null;
            }
        }
        ...
        document.getElementById('toggleWidgets').addEventListener('click', function() {
            ...
            this.innerHTML = active ? '<i class="bi bi-check2"></i> Done' :
                '<i class="bi bi-grid-3x3-gap"></i> Arrange';
    ...
    <script>
        ...
        function updateChart(type, days) {
            $.get('/admin/dashboard/chart', {
                ...
                txChart.updateSeries([{
                    name: 'Volume ($)',
                    data: data.amounts
                }]);
                txChart.updateOptions({
                    xaxis: {
